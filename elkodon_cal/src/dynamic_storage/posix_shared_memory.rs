...
impl<T: Send + Sync + Debug> DynamicStorageBuilder<T, Storage<T>> for Builder<T> {
    ...
    fn create_and_initialize<F: FnOnce(&mut T, &mut BumpAllocator) -> bool>(
        ...
        let mut shm = match SharedMemoryBuilder::new(&full_name)
            ...
            .size(std::mem::size_of::<T>() + self.supplementary_size)
        ...
        let value = shm.base_address().as_ptr() as *mut T;
        unsafe { value.write(initial_value) };

        let supplementary_start =
            (shm.base_address().as_ptr() as usize + std::mem::size_of::<T>()) as *mut u8;
        let supplementary_len = shm.size() - std::mem::size_of::<T>();
        ...
        if !initializer(unsafe { &mut *value }, &mut allocator) {
            fail!(from self, when shm.set_permission(Permission::OWNER_WRITE | Permission::OWNER_READ),
                with DynamicStorageCreateError::Creation,
                "{} since the initialization failed and the permissions could not be adjusted to be able to remove the underlying resources.", msg);
        ...
        fail!(from self, when shm.set_permission(FINAL_PERMISSIONS),
            with DynamicStorageCreateError::Creation,
            "{} since the final permissions {} could not be set.", msg, FINAL_PERMISSIONS);
    ...
    fn try_open(self) -> Result<Storage<T>, DynamicStorageOpenError> {
        ...
        let permission = fail!(from self, when shm.permission(), with DynamicStorageOpenError::InternalError,
            "{} due to a failure while acquiring the current access permissions of the underlying shared memory.", msg);
        if permission != FINAL_PERMISSIONS {
            return Err(DynamicStorageOpenError::InitializationNotYetFinalized);
        }

        let required_size = std::mem::size_of::<T>() + self.supplementary_size;
...
impl<T: Send + Sync + Debug> DynamicStorage<T> for Storage<T> {
    ...
    fn get(&self) -> &T {
        unsafe { &*(self.shm.base_address().as_ptr() as *const T) }
    }
