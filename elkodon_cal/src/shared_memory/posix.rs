    ...
    crate::shared_memory::SharedMemoryBuilder<Allocator, Memory<Allocator>> for Builder<Allocator>
{
    ...
    fn create(
        ...
        let mgmt_size = self.allocator_size(allocator_config);

        let mut shm = match elkodon_bb_posix::shared_memory::SharedMemoryBuilder::new(unsafe {
            &FileName::new_unchecked(self.config.path_for(&self.name).file_name())
        })
        .is_memory_locked(self.config.is_memory_locked)
        .creation_mode(CreationMode::CreateExclusive)
        .size(self.size + mgmt_size)
        ...
        let slice = unsafe {
            std::slice::from_raw_parts_mut(
                (allocator_addr as usize + mgmt_size) as *mut u8,
                self.size,
            )
        };
        ...
        if let Err(v) = shm.set_permission(self.config.permission) {
            fail!(from self, with SharedMemoryCreateError::InternalError,
                    "{} since the configured shared memory permission could not be applied ({:?}).", msg,v);
        }
    ...
    fn open(self) -> Result<Memory<Allocator>, SharedMemoryOpenError> {
        ...
        let permissions = fail!(from self, when shm.permission(),
                        with SharedMemoryOpenError::InternalError,
                        "{} due to a failure while reading the shared memory permissions.", msg);

        if permissions == Permission::OWNER_WRITE {
            fail!(from self, with SharedMemoryOpenError::InternalError,
