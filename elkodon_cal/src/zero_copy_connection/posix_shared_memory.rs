...
impl SharedManagementData {
    ...
    const fn const_memory_size(
        ...
        // we do not have to considuer the alignment of Self since posix shared memory is always
        // page size aligned
    ...
    fn create_or_open_shm(&self) -> Result<SharedMemory, ZeroCopyCreationError> {
        ...
        match shm.has_ownership() {
            true => {
                ...
                fail!(from self, when shm.set_permission(Permission::OWNER_ALL),
                        with ZeroCopyCreationError::InternalError, "{} since the permission could not be updated.", msg);
            ...
            false => {
                ...
                while fail!(from self, when shm.permission(), with ZeroCopyCreationError::InternalError,
                                 "{} since the underlying permissions could not be verified.", msg)
                .contains(Permission::OWNER_READ | Permission::OWNER_WRITE)
                {
                ...
                if !fail!(from self, when shm.permission(), with ZeroCopyCreationError::InternalError,
                                 "{} since the underlying permissions could not be verified.", msg)
                .contains(Permission::OWNER_READ | Permission::OWNER_WRITE)
                {
                    fail!(from self, with ZeroCopyCreationError::ConnectionMaybeCorrupted,
                            "{} since the creating process did not finalize the creation of the connection. Maybe the connection is corrupted?", msg);
                }
