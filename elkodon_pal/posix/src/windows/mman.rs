...
unsafe fn shm_file_path(name: *const char, suffix: &[u8]) -> [u8; MAX_PATH_LENGTH] {
    ...
    // path
    for i in 0..SHM_STATE_DIRECTORY.len() {
        state_file_path[i] = SHM_STATE_DIRECTORY[i];
    }

    // name
    let mut name_len = 0;
    for i in 0..usize::MAX {
        let c = *(name.offset(i as isize) as *const u8);

        state_file_path[i + SHM_STATE_DIRECTORY.len()] = if c == b'/' { b'\\' } else { c };
        if *(name.offset(i as isize)) == 0i8 {
            name_len = i;
            break;
        }
    }
...
pub(crate) unsafe fn shm_set_size(fd_handle: HANDLE, shm_size: u64) {
    ...
    win32call! {SetFilePointer(fd_handle, 0, 0 as *mut i32, FILE_BEGIN)};
    win32call! { WriteFile(
        fd_handle,
