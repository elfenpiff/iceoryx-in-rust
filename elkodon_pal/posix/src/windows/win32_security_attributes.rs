...
fn get_owner_sid(handle: HANDLE) -> [u8; SID_LENGTH] {
    ...
    let mut security_descriptor: PSECURITY_DESCRIPTOR = 0 as *mut void;
    ...
    let mut owner_sid = 0 as *mut void;
    ...
    for i in 0..SID_LENGTH {
        let c = unsafe { *owner_str.offset(i as isize) };
        if c == b'\0' {
            break;
        }
        sid[i] = c;
...
pub fn from_mode_to_security_attributes(handle: HANDLE, mode: mode_t) -> SECURITY_ATTRIBUTES {
    let mut attr = SECURITY_ATTRIBUTES {
        ...
        lpSecurityDescriptor: 0 as *mut void,
...
pub fn from_security_attributes_to_mode(value: &SECURITY_ATTRIBUTES) -> mode_t {
    ...
    for entry in ace_entries {
        ...
        if account_sid == IDENT_OTHERS {
            mode = mode | ace_rights_to_bits(ace_rights) as u64;
        } else if account_sid == IDENT_GROUP {
            mode = mode | ((ace_rights_to_bits(ace_rights) as u64) << 3);
        } else if is_owner(account_sid) {
            mode = mode | ((ace_rights_to_bits(ace_rights) as u64) << 6);
        }
    }
