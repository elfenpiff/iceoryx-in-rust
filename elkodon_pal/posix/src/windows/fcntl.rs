    ...
    match HandleTranslator::get_instance().get(fd) {
        Some(FdHandleEntry::Handle(handle)) => {
            ...
            if handle.state_handle != INVALID_HANDLE_VALUE {
                ...
                let size = win32call! {GetFileSize(handle.handle, 0 as *mut u32)};
            ...
            match acquire_mode_from_path(&file_path) {
                ...
                Some(mode) => {
                    file_stat.st_mode = file_stat.st_mode | mode;
                    buf.write(file_stat);
...
pub unsafe fn fcntl(fd: int, cmd: int, arg: *mut flock) -> int {
    match HandleTranslator::get_instance().get(fd) {
        Some(FdHandleEntry::Handle(handle)) => {
            ...
            if win32call! {LockFileEx(handle.handle, flags, 0, 0xffffffff, 0xffffffff, 0 as *mut OVERLAPPED)}
                == FALSE
            {
                Errno::set(Errno::EINVAL);
                return -1;
